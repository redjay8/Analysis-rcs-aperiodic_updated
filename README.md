...
* **`Umbrella/Data/`**:
    ...
    * **Organization**: Data is further organized into subdirectories named `RCS02`, `RCS03`, ..., up to `RCS20`.
        * Each `RCSXX` folder (e.g., `RCS05`) corresponds to a specific dataset or experimental run. For instance, a path like `/Users/xx/Library/Working/Data/RCS05 3 day sprint` would map to the conceptual `Umbrella/Data/RCS05/` directory in this project structure if `Umbrella/Data/` is symbolically linked or directly points to `/Users/xx/Library/Working/Data/`.
        * **Important Note**: While your local absolute path might be something like `/Users/xx/Library/Working/Data/RCS05 3 day sprint`, within the context of this project structure (especially if you are sharing or collaborating), it's often assumed that code within the `Code/` folder will expect data to be in a relative path like `../Data/RCS05/`. You might need to configure your scripts to correctly locate these data folders, possibly using relative paths or configuration files.

### How to Implement and Use

1.  **Clone the Repository**: When you clone or download this project, ensure the `Umbrella/` folder is your main project directory.
2.  **Place Data**:
    * Ensure your data is organized into the respective `RCS02` through `RCS20` subfolders within the `Umbrella/Data/` directory.
    * If your data resides elsewhere on your system (like `/Users/xx/Library/Working/Data/`), you might need to:
        * Copy the `RCS*` folders into the `Umbrella/Data/` directory.
        * Create symbolic links from `Umbrella/Data/RCSXX` to your actual data locations. For example, on macOS/Linux:
            ```bash
            # Navigate to your project's Data directory
            cd path/to/your/Umbrella/Data/

            # Create a symbolic link for RCS05
            ln -s "/Users/xx/Library/Working/Data/RCS05 3 day sprint" RCS05
            # Repeat for other RCS folders as needed
            ```
